...
| `Makefile` | `make test` (lib-env + badge + config-check unit tests), `make lint` (bash -n + shellcheck + reference check over every shell script; ruff rules/format, mypy strict, yamllint over Python and CI YAML) |
